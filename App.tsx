...
const App: React.FC = () => {
  ...
  const loadData = useCallback(async (ticker: string) => {
    ...
    try {
      const details = await fetchTickerDetails(ticker, POLYGON_API_KEY);
      const history = await fetchHistoricalData(ticker, POLYGON_API_KEY);
      const quote = await fetchFinnhubQuote(ticker);
      const isMockData = details.description.includes('simulated data');
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-50 flex flex-col">
      ...
      <main className="flex-1 flex flex-col lg:flex-row h-[calc(100vh-64px)] overflow-hidden min-h-0">
        {state.error ? (
